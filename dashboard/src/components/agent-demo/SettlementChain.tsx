...
export default function SettlementChain({ settlements }: Props) {
  ...
  return (
    <div style={{ background: 'var(--lf-surface)', border: '1px solid var(--lf-border)', borderRadius: 12, overflow: 'hidden' }}>
      <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--lf-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <div className="t-label">On-chain settlement chain (Solana)</div>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--f-mono-2)', fontSize: 12 }}>
          <thead>
          ...
          <tbody>
            {settlements.map((s, i) => (
              <tr key={s.jobId + i} style={{ borderTop: '1px solid var(--lf-border)', color: 'var(--lf-ink)' }}>
                <td style={td}>{i + 1}</td>
                <td style={td}>
                  <span style={{ color: 'var(--lf-ink-3)' }}>#{s.skillId}</span> {s.skillName}
                </td>
                ...
                <td style={td}>
                  <a href={s.explorerUrl} target="_blank" rel="noopener noreferrer"
                    ...
                    {s.settlementSignature ? `${s.settlementSignature.slice(0, 12)}…` : 'view on explorer ↗'}
                  </a>
                </td>
              </tr>
            ))}
...
const th: React.CSSProperties = {
  textAlign: 'left', fontWeight: 500, textTransform: 'uppercase',
  fontSize: 10, letterSpacing: '0.08em', padding: '10px 14px',
}
