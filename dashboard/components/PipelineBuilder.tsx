    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter items-start">
      ...
      <aside className="lg:col-span-4 flex flex-col gap-10 lg:sticky lg:top-[88px]">
        ...
        <section className="border border-ink p-6 flex flex-col gap-5">
          ...
          {result && (
            <div style={{ border: `1px solid ${C.green}`, padding: 14, background: "#FBEFEE" }}>
              ...
              {earlier.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 0, border: `1px solid ${C.line}` }}>
                  {earlier.map((e, i) => {
                    ...
                    return (
                      <button key={e.id} onClick={() => toggleDep(selNode.id, e.id)} className="mono" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 10px", borderTop: i > 0 ? `1px solid ${C.line}` : "none", fontSize: 12, cursor: "pointer", border: "none", background: on ? "#FBEFEE" : "transparent", color: on ? C.green : C.tx }}>
